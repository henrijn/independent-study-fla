...
class PhoenixAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.g_norm = RMSNorm(self.value_dim, elementwise_affine, eps=norm_eps, dtype=torch.float32)
    ...
    def forward(
        ...
        **kwargs,
        ...
        r = self.r_proj(hidden_states)

        q = rearrange(q, '... (h d) -> ... h d', d=self.head_k_dim)
        k = rearrange(k, '... (h d) -> ... h d', d=self.head_k_dim)
        v = rearrange(v, '... (h d) -> ... h d', d=self.head_v_dim)
        r = rearrange(r, '... (h m) -> ... h m', m=self.num_slots)
        ...
        if self.training and self.router_noise:
            x = torch.empty_like(r).exponential_(1)
            r = r - torch.log(x + 1e-6)
        ...
        r = mask / (mask.sum(dim=-1, keepdim=True) + 1e-6)

        g = a.unsqueeze(-1) * r
        s = (1 - g.exp()).to(g.dtype)

        if self.num_kv_groups > 1:
            k, v, g, s = map(lambda x: repeat(x, '... h d -> ... (h g) d', g=self.num_kv_groups), (k, v, g, s))
